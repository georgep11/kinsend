...
import { getImportContactHistoryAsync, selectSettings } from "../../redux/settingsReducer";
        ...
        contactHistory?.map((history, index) => (
          <div className={`flex py-4 justify-between items-center border-gray-1 ${ index === contactHistory.length - 1 ? '' : ' border-b-1 ' }`} key={index}>
            ...
            <div className="flex-grow ml-5">
              <p className="font-bold text-base">Import: {format(new Date(history.createdAt), "MMM dd, hh:mm aa")}</p>
