...
const fieldNotSelected = (mappedFields, field) => {
  return (
    mappedFields.filter((mappedField) => mappedField.to === field).length === 0
  );
};
...
const FieldSelectModal = ({ close, visible, fieldName, onSelect }) => {
  ...
  const fields = useMemo(() => {
    return [...DEFAULT_FIELDS, ...OPTION_FIELDS].filter(
      (field) =>
        field.toLowerCase().indexOf(searchText) >= 0 &&
        fieldNotSelected(mappedFields, field)
    );
  ...
  return (
    <>
      ...
      >
        <h3 className="font-bold text-center text-xl md:text-4xl mb-5">
          Map Column {fieldName} to
        </h3>
        <div className="mt-10">
          <div className="flex">
            <Search
          ...
          <div className="mt-10">
            {fields.map((field, index) => (
              <div
                key={index}
                className="flex flex-row justify-between items-center py-2 border-t-1 border-gray-1"
              >
                <div>
                  <strong>{field}</strong>
                </div>
