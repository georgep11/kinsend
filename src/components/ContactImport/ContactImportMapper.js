    ...
    <div className="bg-gray-1 p-7 md:p-12 border-4 border-white">
      <div className="flex md:flex-row flex-col justify-between">
        <div>
          <h2 className="text-lg text-black font-bold mb-1">
            Map your File to your Kinsend
          </h2>
          {isCurrentStep && (
            <p className="mt-2 md:mt-0 text-center md:text-left md:max-w-sm lg:max-w-full">
              Select which data you want to match to your file columns by
              selecting each from the dropdown.
            </p>
          )}
        </div>
        <div>
          <p className="mt-2 md:mt-0 text-center md:text-left">
            <span>
              <strong>{colValues[0].length}</strong>{" "}
              {colValues[0].length > 1 ? "Rows" : "Row"},{" "}
              <strong>{mappedFields.length}</strong> Columns Mapped
            </span>
            {!isCurrentStep && (
              <span
      ...
      {isCurrentStep && (
        <div className="w-full overflow-x-auto mt-5">
          <div
            className={`flex flex-row space-x-5 my-5`}
      ...
      {!isRequiredFieldsMapped && (
        <p className="mt-5 text-red-500">
          Mapping is incorrect! Ensure that your table includes columns for
